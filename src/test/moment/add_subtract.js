...
test('add across DST', function (assert) {
    ...
    var a = moment(new Date(2011, 2, 12, 5, 0, 0)), b, c, d, e;
    b = a.add(1, 'days');
    c = a.add(24, 'hours');
    d = a.add(1, 'months');
    e = a.add(1, 'quarter');

    assert.equal(b.hours(), 5, 'adding days over DST difference should result in the same hour');
    if (c.isDST() && !a.isDST()) {
        assert.equal(c.hours(), 6, 'adding hours over DST difference should result in a different hour');
    } else if (!c.isDST() && a.isDST()) {
        assert.equal(c.hours(), 4, 'adding hours over DST difference should result in a different hour');
    } else {
        assert.equal(c.hours(), 5, 'adding hours over DST difference should result in a same hour if the timezone does not have daylight savings time');
    }
    assert.equal(d.hours(), 5, 'adding months over DST difference should result in the same hour');
